...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.type === "GET_DAILY_INCOME") {
    ...
    handleGetDailyIncomeWithCache({ posts: postsWithIncome, username }).then(
      sendResponse,
    );
    return true;
  }
  ...
  if (request.type === "GET_MONTHLY_STATA_READS_VIEWS") {
    ...
    handleGetMonthlyStatsReadsView({ username, startTime, endTime }).then(
      sendResponse,
    );
    return true;
  }
  ...
  if (request.type === "GET_POST_STATS_DAILY_BUNDLE") {
    ...
    handleGetAllPostsStatsDailyBundle({ postsIds, startTime, endTime }).then(
      sendResponse,
    );
    return true;
  }
...
function handleGetNotificationsRecursive({
  pagingOptions,
  results = [],
  endDate,
  depth = 0,
  ...
  return client
    ...
    .then(({ data }) => {
      const notifications =
        data.notificationsConnectionByActivityTypes.notifications;
      ...
      const nextPage =
        data.notificationsConnectionByActivityTypes.pagingInfo.next;
...
async function handleGetAllPostsStatsDailyBundle({
  postsIds,
  startTime,
  endTime,
}) {
  ...
  for (const postId of postsIds) {
    const data = await handleGetPostStatsDailyBundleWithCache({
      postId,
      startTime,
      endTime,
    });
...
async function handleGetPostStatsDailyBundleWithCache({
  postId,
  startTime,
  endTime,
}) {
  ...
  const data = await handleGetPostStatsDailyBundle({
    postId,
    startTime,
    endTime,
  });
...
function handleGetDailyIncome({ posts }) {
  ...
  return Promise.all(incomePromises).then((incomeArray) => {
    return incomeArray;
  });
}
...
async function handleGetStats({ username, after = "", depth = 0 }) {
  ...
  const nextPageData =
    pageInfo?.hasNextPage && depth < MAX_RECURSION_DEPTH
      ...
      : {
          list: [],
          totals: { reads: 0, views: 0, claps: 0, responses: 0, income: 0 },
        };
